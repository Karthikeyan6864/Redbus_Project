...
if tabs == "Home":
    ...
    min_price, max_price = int(final_filtered_df['price'].min()), int(final_filtered_df['price'].max())

    if min_price == max_price:
        st.write(f"Only one price available: ₹{min_price}")
        selected_price_range = (min_price, max_price)
    else:
        selected_price_range = st.slider(
                                  'Select Price Range (Step 5)',
                                            min_value=min_price,
                                            max_value=max_price,
    ...
    final_filtered_df = final_filtered_df[
                (final_filtered_df['price'] >= selected_price_range[0]) & 
                (final_filtered_df['price'] <= selected_price_range[1])
                                          ]


    st.write(f"You selected {selected_Bus_route}, {selected_Bus_type}, {selected_Rating}, {selected_Duration}, Price range: {selected_price_range}")

    if final_filtered_df.empty:
        st.write("No records found for the selected criteria and price range.")
    else:
        st.write("Filtered Data:")
        columns_to_display = ['busname','bustype','duration','star_rating', 'price', 'seats_available', 'departing_time', 'reaching_time', 'route_link']
